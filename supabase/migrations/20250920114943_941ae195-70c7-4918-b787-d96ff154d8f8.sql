...
-- Update existing students table to reference profiles
...
ALTER TABLE public.students DROP COLUMN IF EXISTS "photo url";
ALTER TABLE public.students ADD COLUMN IF NOT EXISTS profile_id UUID REFERENCES public.profiles(id) ON DELETE CASCADE;

-- Update existing teachers table to reference profiles  
ALTER TABLE public.teachers DROP COLUMN IF EXISTS email;
ALTER TABLE public.teachers DROP COLUMN IF EXISTS name;
ALTER TABLE public.teachers ADD COLUMN IF NOT EXISTS profile_id UUID REFERENCES public.profiles(id) ON DELETE CASCADE;

-- Update attendance table to properly reference profiles
ALTER TABLE public.attendance RENAME COLUMN student_id TO profile_id;
ALTER TABLE public.attendance ADD CONSTRAINT fk_attendance_profile 
